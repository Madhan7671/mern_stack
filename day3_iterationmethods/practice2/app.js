...
const createcard = () => {
    data.recipes.forEach((recipe) => {
        ...
        newclass.innerHTML = `<h1>${recipe.name}</h1><h4>${recipe.mealType}</h4><h2>${recipe.caloriesPerServing} cal</h2>
        <br><image src="${recipe.image}" alt="${recipe.name}" class="recipe-img" weidth="100%" height=180px><h3>⭐${recipe.rating}  (${recipe.reviewCount}) 
        ⌛${recipe.prepTimeMinutes} min
        📌${recipe.cuisine}</h3>`;
        parent.append(newclass);
    });
}
createcard();